...
class ImageProcessorSerial {
  final String inputPath;
  final String outputPath;

  ImageProcessorSerial({
  ...
  Future<void> _processImage(FileSystemEntity fileSystemEntity) async {
    ...
    for (int y = 0; y < image.height; y++) {
      for (int x = 0; x < image.width; x++) {
        ...
        final grayPixel = (255 << 24) | (gray << 16) | (gray << 8) | gray;  // Pixel em escala de cinza
    ...
    final outputFile = File('assets/converted_images/converted_${file.path.split('/').last}');
...
void main() async {
  final processor = ImageProcessorSerial(
    inputPath: 'assets/images',  // Caminho de entrada
    outputPath: 'assets/converted_images',  // Caminho de saída
  );
